...
MRWIDE_API_URL = os.getenv("MRWIDE_API_URL", "http://127.0.0.1:8001")

_mem_seq = 0
_mem_tickets: dict[int, dict] = {}
_mem_replies_by_user: dict[str, list[dict]] = {}

MRWIDE_TICKET_THRESHOLD = 10000
...
def _save_ticket(t: dict):
    ...
    if r:
        try:
            ...
            r.expire(_redis_ticket_key(tid), 60 * 60 * 24 * 14)
            return
        except Exception:
            pass
    _mem_tickets[tid] = t
...
def _forward_to_mrwide(update: dict) -> dict:
    """Forward Telegram webhook to Mr.Wide API."""
    try:
        resp = requests.post(
            f"{MRWIDE_API_URL}/api/telegram/webhook",
            json=update,
            timeout=15,
        )
        return resp.json()
    except Exception as e:
        logger.error(f"Mr.Wide forward error: {e}")
...
def telegram_webhook():
    ...
    if msg.startswith("/resolve"):
        ...
        # Mr.Wide tickets: id >= 10000
        if tid >= MRWIDE_TICKET_THRESHOLD:
            result = _forward_to_mrwide(update)
            return jsonify(result)
        ...
        t = _load_ticket(tid)
        if not t:
            # Not found in TornadoAI either -> try Mr.Wide as fallback
            result = _forward_to_mrwide(update)
            return jsonify(result)
